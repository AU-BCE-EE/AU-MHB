...
mm <- du[FertiliserType == 'Urea', .(x1 = RefKey - 0.9, x2 = RefKey + 0.9, med = median(NH3loss), mn = mean(NH3loss), mx = max(NH3loss), n = length(NH3loss)), by = .(Ref, RefKey)]
setorder(mm, med)
xlabs <- unique(du[, .(RefKey, Ref)])
set.seed(1)
ggplot(du, aes(RefKey, NH3loss, colour = factor(RefKey))) + 
  geom_segment(data = mm, aes(x = x1, xend = x2, y = med, yend = med)) +
  geom_jitter(height = 0, alpha = 0.4, size = 0.5) +
...
mm <- du[, .(x1 = RefKey - 0.9, x2 = RefKey + 0.9, med = median(NH3loss), mn = mean(NH3loss), mx = max(NH3loss), n = length(NH3loss)), by = .(Ref, RefKey, FertiliserType)]
xlabs <- unique(du[, .(RefKey, Ref)])
