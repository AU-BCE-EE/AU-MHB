...
dat$id <- interaction(dat$animal, dat$sample, dat$month)
dat <- droplevels(dat)
...
for (x in split(dat, dat$id)) {
  ...
  dd <- data.frame(animal = x$animal[1], sample = x$sample[1], month = x$month[1], id = x$id[1], pH = ddll, e.rel = y1, red = y2)
  dd <- na.omit(dd)
  pHidat <- rbind(pHidat, dd)
}

# Summarize by animal and pH level
pH.summ <- as.data.frame(summarise(group_by(pHidat, animal, month, pH),
...
for (x in split(dat, dat$id)) {
  ...
  dd <- data.frame(animal = x$animal[1], sample = x$sample[1], month = x$month[1], id = x$id[1], dpH = ddll, e.rel = y1, red = y2)
  dd <- na.omit(dd)
  dpHidat <- rbind(dpHidat, dd)
}

# Summarize by animal and dpH
dpH.summ <- as.data.frame(summarise(group_by(dpHidat, animal, month, dpH),
                                n = length(unique(sample)),
                                e.rel.m = mean(e.rel), e.rel.s = sd(e.rel), e.rel.mn = min(e.rel), e.rel.mx = max(e.rel),
                                red.m = mean(red), red.s = sd(red), red.mn = min(red), red.mx = max(red)))
