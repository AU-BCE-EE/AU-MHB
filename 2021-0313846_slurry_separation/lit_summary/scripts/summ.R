...
# Final summary
dmds2 <- aggregate(dmdw1[, c('DM.raw', 'DM.liquid', 'DM.solid', 'dDM.liquid', 'dDM.solid')], 
                 by = dmdw1[, c('sep.scale', 'slurry.source')], FUN = mean, na.rm = TRUE)
names(dmds2)[-1:-2] <- paste0(names(dmds2)[-1:-2], '.mean')
dmsds2 <- aggregate(dmdw1[, c('DM.raw', 'DM.liquid', 'DM.solid', 'dDM.liquid', 'dDM.solid')], 
                 by = dmdw1[, c('sep.scale', 'slurry.source')], FUN = sd, na.rm = TRUE)
names(dmsds2)[-1:-2] <- paste0(names(dmsds2)[-1:-2], '.sd')
dmns2 <- aggregate(dmdw1[, c('DM.raw', 'DM.liquid', 'DM.solid', 'dDM.liquid', 'dDM.solid')], 
