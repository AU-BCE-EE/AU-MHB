...
dat$app.mthd.nm <- factor(paste0(dat$app.mthd, ifelse(dat$app.mthd == 'Trailing hose', paste0(' + ', dat$acid), '')), 
                            levels = c('Trailing hose + 0 kg/t', 'Trailing hose + 2.1 kg/t', 'Trailing hose + 5.7 kg/t', 'Trailing hose + 11 kg/t', 'Open slot injection', 'Closed slot injection'),
...
dat.acomp$man.source.nm <- factor(dat.acomp$man.source, levels = c('Cattle', 'Pig', 'Digestate'))
